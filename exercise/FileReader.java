...
public class FileReader 
{
	...
	private void populateSizeBits() {
        ...
        for (int i = 0; i < sizeBitsLength; i++) {
            sbits.add((size & (0x1 << i)) != 0 ? 0x1 : 0x0);      
        } 
	...
	private void populateExtensionBits() {
        ...
        if (extensionBytes.length * byteLength > extBitsLength) {
            System.err.println("File extension is of length " +  
                extensionBytes.length * byteLength + " bits. The file extension " + 
                "cannot be greater than " + extBitsLength + " bits");
            
            success = false;
            return;
        }

        for (int byteNo = 0; byteNo < extensionBytes.length; byteNo++) {
            for (int bitNo = 0; bitNo < byteLength; bitNo++) {
                extBits.add((extensionBytes[byteNo] & (0x1 << bitNo)) != 0 ? 0x1 : 0x0);
            }
        }
        
        // Pad unused bits with 0s
        for (int bitNo = extBits.size(); bitNo < extBitsLength; bitNo++) {
            extBits.add(0x0);
        }
